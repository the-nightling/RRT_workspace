...
%    iter = 1;
	...
	for n = 2:N
		...
		%%dsq = euclidianDistSquare(x_rand,V(:,1:n));
		
		%% select RRT vertex closest to the state point
		%%[~,i] = min(dsq);
		
		% LQR_Nearest(V,x_rand)
		...
		for ui = 1:length(U)
			x_rand_c(:,ui) = V(:,i) + dt*dynamics(V(:,i),U(ui));
		end
		
		% select the closest reachable state point
		dsq = euclidianDistSquare(x_rand,x_rand_c);
		[~,ui] = min(dsq);
		x_rand = x_rand_c(:,ui);
		...
		temp = x_rand(1);
		if( (x_rand(1) > pi) || (x_rand(1) < -pi) )
        		x_rand(1) = mod(x_rand(1)+pi,2*pi)-pi;
        end
		
		% plot new RRT branch
		if(abs(x_rand(1)-temp) < pi)
			line([V(1,i),x_rand(1)],[V(2,i),x_rand(2)],'Color','b');
		end
		...
		if(mod(n,250)==1)
			drawnow;
%			f=getframe;
%            im(:,:,1,iter) = rgb2ind(f.cdata,map);
%            iter = iter+1;
		end

		% if the goal was reached, stop growing tree
		if((x_rand(1) <= xG(1)+0.1) && (x_rand(1) >= xG(1)-0.1))
		    if((x_rand(2) <= xG(2)+0.5) && (x_rand(2) >= xG(2)-0.5))			
			    break;
			end
		end
...
function x_nearest_index = LQR_Nearest(V,x_rand,n)

    [A,B] = linearize_pendulum(x_rand);
    Q = diag([1 10]);
    R = 1;
    [K,S] = lqr(A,B,Q,R);
    ...
    for i = 1:n
        new_cost_to_go = [V(:,i)-x_rand]' * S * [V(:,i)-x_rand];
...
function [A,B] = linearize_pendulum(x)
    g = 9.8;
    b = 0.1;
    l = 1;
    m = 1;

    % linearized dynamics about x_rand
    A = [0 1; (g*sin(x(1)))/l -b/(m*l*l)];
    B = [0; 1/(m*l*l)];

end

function sigma = LQR_Steer(x_nearest, x_rand)
